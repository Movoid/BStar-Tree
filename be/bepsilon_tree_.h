...
private:
  ...
  void do_1_2_split_root_() noexcept {
    ...
    if (root->is_leaf) {
      ...
      std::memcpy(node2->key, root->key + need1, need2 * sizeof(key_type));
      std::memcpy(node2->leaf.data_ptr, root->leaf.data_ptr + need1, need2 * sizeof(val_type *));
      ...
      std::memcpy(node2->key, root->key + need1 + 1, need2 * sizeof(key_type));
      std::memcpy(node2->idx.key_ptr, root->idx.key_ptr + need1 + 1, (need2 + 1) * sizeof(node_type *));
  ...
  void do_2_1_merge_root_() noexcept {
    ...
    if (node1->is_leaf) {
      // key
      std::memmove(root->key, node1->key, node1->key_cnt * sizeof(key_type));
      std::memmove(root->key + node1->key_cnt, node2->key, node2->key_cnt * sizeof(key_type));
      // ptr
      std::memmove(root->leaf.data_ptr, node1->leaf.data_ptr, node1->key_cnt * sizeof(val_type *));
      std::memmove(root->leaf.data_ptr + node1->key_cnt, node2->leaf.data_ptr, node2->key_cnt * sizeof(val_type *));
      ...
      std::memmove(root->key + node1->key_cnt + 1, node2->key,
                   node2->key_cnt * sizeof(key_type)); // ?
      // ptr
      std::memmove(root->idx.key_ptr, node1->idx.key_ptr, (node1->key_cnt + 1) * sizeof(node_type *));
      std::memmove(root->idx.key_ptr + node1->key_cnt + 1, node2->idx.key_ptr,
                   (node2->key_cnt + 1) * sizeof(node_type *));
